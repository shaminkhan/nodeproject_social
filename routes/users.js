...
cloudinary.config({
	cloud_name: 'dznrav1fq',
	api_key: '527249366189859',
	api_secret: 'LoTFs0ZevysHma2RzsmMTSFMd7o'
});
...
router.get('/delete/:id',function(req,res,next){
	...
	Message.find({ $or: [{ sender:req.params.id },
		{ receiver:req.params.id }] }).remove().exec();
...
router.post('/upload',upload.single('upload'),function(req,res,next){
	...
	if(req.file){
		cloudinary.uploader.upload("./public/images/profile/"+req.file.filename,
			function(result){
			User.findOneAndUpdate({_id:req.user._id},{$set:{image:result.secure_url}},
				(err,user) => {
		...
		res.send({msg:"Oops! make sure image should be in .jpeg or .png format.",
			success:false});
	}
});
...
router.post('/update',middleware.update_Valid,function(req,res,nex){
	...
	User.findOneAndUpdate({_id:req.user._id},
		{$set:{fullname:req.body.fullname,username:req.body.username,bio:req.body.bio}},
		(err,user) => {
		if(err) res.send({msg:"Something went wrong",success:false});
		if(user){
